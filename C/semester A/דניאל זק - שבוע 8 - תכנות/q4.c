...
void takeOrder(void)
{
	...
	getchar();// for cleaning the buffer
	
	printf("Choose size\n %c-big\n%c-small\n", BIG_CHOICE, SMALL_CHOICE);
	scanf("%c",&sizeChoice); // change d to c

	printf("how many would you like (1-9)?\n");
	scanf("%d", &howMany);
	
	printf("Do you have a tlush? click 0 for no, any other number for yes.\n");
	scanf("%d", &hasTlush);// ampesand

	printReceipt(typeChoice, sizeChoice, howMany, hasTlush);//change between size and type
