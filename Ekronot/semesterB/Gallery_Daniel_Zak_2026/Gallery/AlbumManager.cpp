...
void AlbumManager::showPicture()
{
	...
	std::cout << "Chose an application to open the picture (enter the number):" << std::endl;
	std::cout << "1. Default application (mspaint.exe)" << std::endl;
	std::cout << "2. IrfanView (if installed)" << std::endl;
	std::string appChoice = getInputFromConsole("Your choice: ");
	...
	if (appChoice == "1"){
		...
		appPath = "C:\\Program Files\\IrfanView\\i_view64.exe";
		if (!fileExistsOnDisk(appPath)) {
			throw MyException("Error: IrfanView is not installed on this system.\n");
		}
	}
	else {
		throw MyException("Error: Invalid application choice.\n");
	}


	//we want to open with ms paint for now
	std::string command = appPath + " \"" + pic.getPath() + "\"";
	...
	if (!CreateProcessA(nullptr, commandLine.data(), nullptr, nullptr, FALSE, 0, nullptr, nullptr, &si, &pi)) {
		throw MyException("Error: Failed to open picture <" + picName + ">. Error code: " + std::to_string(GetLastError()) + "\n");
	}

	hChildProcess = pi.hProcess; // store the handle to the child process
	WaitForSingleObject(pi.hProcess, INFINITE);
	hChildProcess = nullptr; // reset the handle after the process has exited
	CloseHandle(pi.hProcess);
	CloseHandle(pi.hThread);
	
	std::cout << "returning back to the Menu..." << std::endl;
}
...
void AlbumManager::removeUser()
{
	...
	//V1.0.1 Bugfix - make sure to remove user from everywhere
	const User& user = m_dataAccess.getUser(userId);
	...
	for (const auto& album : m_dataAccess.getAlbums()) {
		if (album.getOwnerId() == userId) {
			m_dataAccess.deleteAlbum(album.getName(), album.getOwnerId());
			continue;
		}
		else {
			for (const auto& picture : album.getPictures()) {
				if (picture.isUserTagged(user)) {
					m_dataAccess.untagUserInPicture(album.getName(), picture.getName(), userId);
				}
			}
		}
	}
...
void AlbumManager::userStatistics()
{
	...
	const User& user = m_dataAccess.getUser(userId);
	//V1.0.1 Bugfix - add the amount of albums the user owns to the statistics
	std::cout << "user @" << userId << " Statistics:" << std::endl << "--------------------" << std::endl <<
		"  + Count of Albums Owned: " << m_dataAccess.countAlbumsOwnedOfUser(user) << std::endl << // added this line Tomer :)
