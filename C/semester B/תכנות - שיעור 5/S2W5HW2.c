...
{
	list->listName = (char*)calloc(STR_LEN+1, sizeof(char));
	if (list->listName == NULL)
	{
		printf("Memory allocation failed for list name.\n");
		exit(1);
	}
	...
	{
		list->reasons[i] = (char*)calloc(STR_LEN+1, sizeof(char));
		if(list->listName == NULL);
		{
			printf("Memory allocation failed for list name.\n");
			exit(1);
...
void addReason(reasonList* list)
{
	...
	char answer[STR_LEN] = {0};
	myFgets(answer, STR_LEN);
	strcpy(list->reasons[list->numReasons], answer);
	list->numReasons = list->numReasons + 1;
}
